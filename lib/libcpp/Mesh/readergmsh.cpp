...
ReaderGmsh::ReaderGmsh() : alat::InterfaceBase()
{
...
std::string ReaderGmsh::getClassName() const
{
  return "ReaderGmsh";
}
...
/*---------------------------------------------------------*/
void ReaderGmsh::_read_nodes_binaray(std::ifstream& file)
{
  std::string str;
  int num_nodes, id;
  file >> num_nodes;
...
/*---------------------------------------------------------*/
void ReaderGmsh::_read_nodes_ascii(std::ifstream& file)
{
  std::string str;
  int num_nodes, id;
  file >> num_nodes;
...
/*---------------------------------------------------------*/
void ReaderGmsh::_read_elements_binaray(std::ifstream& file)
{
...
/*---------------------------------------------------------*/
void ReaderGmsh::_read_elements_ascii(std::ifstream& file)
{
  alat::armaivec& col = _elems.col();
  alat::armaivec& rowstart = _elems.rowstart();
...
/*---------------------------------------------------------*/
void ReaderGmsh::setMesh(MeshUnitInterface* mesh)
{
  // std::cerr << "ReaderGmsh::setMesh() _npartsmax="<<_npartsmax<<"\n";
  if(_npartsmax)
  {
  ...
  for(int i = 0; i < _elems.n(); i++)
  {
    ...
    {
      if(color_to_sizebdry1.hasKey(physical) )
      {
        color_to_sizebdry1[physical]++;
      }
      else
      {
        color_to_sizebdry1[physical] = 1;
      }
    }
    else if(type == gmshsideside)
    {
      if(color_to_sizebdry2.hasKey(physical) )
      {
        color_to_sizebdry2[physical]++;
      }
      else
      {
        color_to_sizebdry2[physical] = 1;
      }
    }
    else if(type == gmshsidesideside)
    {
      if(color_to_sizebdry3.hasKey(physical) )
      {
        color_to_sizebdry3[physical]++;
      }
      else
      {
        color_to_sizebdry3[physical] = 1;
      }
    }
  }
...
/*---------------------------------------------------------*/
void ReaderGmsh::_setMeshAndInterfaces(MeshUnitInterface* mesh)
{
  arma::mat& nodes = mesh->getNodesAndNodesOfCells()._nodes;
  alat::armaimat& nodes_of_cells = mesh->getNodesAndNodesOfCells()._nodes_of_cells;
  int gmshcell = mesh->getGmshCellType();
    ...
    if(type == gmshcell)
    {
      ...
      for(int ii = 0; ii < nparts; ii++)
      {
        if(parts[ii] < 0)
        {
          ...
          if(partion_to_sizecells.hasKey(partition) )
          {
            partion_to_sizecells[partition]++;
          }
          else
          {
            partion_to_sizecells[partition] = 1;
          }
        }
      }
    }
