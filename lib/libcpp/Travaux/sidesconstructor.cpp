...
/*---------------------------------------------------------*/
template<int NODESPERSIDE>
void SidesConstructor<NODESPERSIDE>::constructSidesFromCells(const MeshUnitInterface::BoundarySideToColorForSidesConstructor& bstcin, const alat::Map<int, alat::armaivec>* partition_to_cell, int color_default)
{
  ...
  for(int i = 0; i < _nodes_of_cells.n_cols; i++)
  {
    for(int ii = 0; ii < SIDESPERCELL; ii++)
    {
      ...
      if(not foundinparts)
      {
        std::cerr<<"*** not found "<<ssort<< " for cell " << i << "\n";
        // std::cerr << "bstc = " << bstc << "\n";
        // assert(0);
