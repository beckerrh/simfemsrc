...
/*---------------------------------------------------------*/
void test(Solver& solver, std::string testname, meshEnums::meshtype type, std::string method, std::string fem)
{
  double diff=1.0, alpha=1.0, deltasupg=0.0, robin = 10.0,  gamma=1.1;
  ...
  solver.setParameter("robin", robin);
  solver.setParameter("alpha", alpha);
  solver.setParameter("gamma", gamma);
...
int main(int argc, char** argv)
{
  ...
  methods.push_back("traditional");
  methods.push_back("nitsche");
  Solver solver;
  for(std::list<std::string>::const_iterator p=methods.begin();p!=methods.end();p++)
  {
    std::cerr << "##############  " << *p <<  "  ##############\n";
    // test(testname, meshEnums::LineMesh, *p, fem);
    test(solver, testname, meshEnums::TriangleMesh, *p, fem);
